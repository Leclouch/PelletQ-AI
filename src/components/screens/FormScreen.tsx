    ...
    <AppShell>
      ...
      >
        <div style={{ display: 'flex', gap: 7 }}>
          {[1, 2, 3].map(n => (
            <div key={n} style={{ flex: 1, height: 5, borderRadius: 999, background: n <= step ? '#2563EB' : '#E6E0D1', transition: 'background .25s' }} />
          ))}
        </div>
      </StickyHeader>
      ...
      <StickyFooter>
        ...
        <button onClick={onNextStep} disabled={computing} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, padding: 15, borderRadius: 14, background: computing ? '#E8E2D4' : 'linear-gradient(135deg,#2563EB 0%,#1D4ED8 100%)', color: computing ? '#A6AFA7' : '#fff', fontSize: 15.5, fontWeight: 800, cursor: computing ? 'not-allowed' : 'pointer', boxShadow: computing ? 'none' : '0 6px 16px rgba(29,78,216,.24)' }}>
          {step === 3 ? 'Hitung Formulasi ✨' : 'Lanjut →'}
        </button>
      </StickyFooter>

      {computing && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 30, background: 'rgba(255,255,255,.88)', backdropFilter: 'blur(6px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
          <div style={{ width: 58, height: 58, borderRadius: '50%', border: '5px solid #E1EBFB', borderTopColor: '#2563EB', animation: 'spin .8s linear infinite' }} />
