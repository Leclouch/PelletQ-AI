...
export default function IngredientDropdown({ ingredients, selectedId, selectedName, isOpen, onToggle, onSelect }: IngredientDropdownProps) {
  return (
    <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
      <button onClick={onToggle} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, width: '100%', padding: '11px 12px', border: `1.5px solid ${isOpen ? '#1A8A5E' : '#E2DDCE'}`, borderRadius: 11, fontSize: 14, fontWeight: 700, background: '#FCFBF7', cursor: 'pointer', color: selectedName ? '#1C2E27' : '#9AA69E', textAlign: 'left' }}>
      ...
      {isOpen && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, zIndex: 20, background: '#fff', border: '1px solid #E2DDCE', borderRadius: 13, boxShadow: '0 14px 32px rgba(28,46,39,.18)', overflow: 'hidden', maxHeight: 264, overflowY: 'auto' }}>
          ...
          {ingredients.filter(i => i.statusTersedia).map(opt => (
            <button key={opt.id} onClick={() => onSelect(opt.id, opt.name)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, width: '100%', textAlign: 'left', padding: '11px 13px', fontSize: 13.5, fontWeight: selectedId === opt.id ? 800 : 600, color: selectedId === opt.id ? '#15724B' : '#3A4742', background: selectedId === opt.id ? '#E4F1E9' : '#fff', cursor: 'pointer', borderBottom: '1px solid #F2EEE2' }}>
              {opt.name}
