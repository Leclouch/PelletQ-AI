...
void handleMqtt() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    mqttOk = false;
    wasMqtt = false;
    if (now - lastRetry >= 5000) {           // retry non-blocking
      lastRetry = now;
  ...
  if (!mqtt.connected()) {
    if (wasMqtt) { Serial.println(F("[mqtt] disconnected")); wasMqtt = false; }
    mqttOk = false;
    if (now - lastRetry >= 5000) {
      ...
      if (mqtt.connect(MQTT_CLIENT_ID, nullptr, nullptr,
        ...
        Serial.printf("[mqtt] connect failed, rc=%d (retry 5s)\n", mqtt.state());
      }
    }
    return;
  }
...
void publishEvent(const char* ev) {
  ...
  char buf[128];
  size_t n = serializeJson(doc, buf, sizeof(buf));
  mqtt.publish(TOPIC_EVENT, (const uint8_t*)buf, n, false);
}
